...
- **保存（`ctrl+s`）**：
  1. 把表单值组装成 `config.Sync`，并入 `config.Config`（编辑=替换同名条目，新增=追加）。
  2. 调 `config.Config.Validate()`。校验失败 → 在表单底部状态行红字显示错误（如「sync "web1-www": local_path is required」），不退出。
  3. 校验通过 → `config.Save(cfgPath, cfg)`，返回主菜单并刷新。
- **取消（`esc`）**：丢弃改动，返回主菜单。
- 端口输入做数字解析；非数字在保存时报错。
...
- **中断（`ctrl+c`）**：用 `context.WithCancel`，取消后 rsync 子进程随 `exec.CommandContext` 被杀；当前条目记为失败，停止后续条目。
...
type App struct {
    ...
    logDir  string
    cfg     *config.Config
    deps    syncer.Deps          // 复用核心层：execx.Real + snapshot.RealFSType + time.Now
    screen  screen
    list    listModel
    form    formModel
    run     runModel
...
- `deps` 用真实实现（`execx.Real{}`、`snapshot.RealFSType`、`time.Now`），与 CLI 相同——这意味着 TUI 调的就是真同步、真快照、真删除。
